...
export const getRentWithoutHeader = async (client: Client, bytes: bigint | number) => {
    const rentForHeader = await client.rpc.getMinimumBalanceForRentExemption(0n).send();
...
export const createDeployedProgram = async (
    ...
    const [buffer, program, dataRent, programRent, defaultTransaction] = await Promise.all([
        generateKeyPairSigner(),
        generateKeyPairSigner(),
    ...
    await pipe(
        defaultTransaction,
        tx => appendTransactionMessageInstructions([createBufferIx, initializeBufferIx, writeIx], tx),
        tx => signAndSendTransaction(client, tx),
    );
    await pipe(
        defaultTransaction,
        tx => appendTransactionMessageInstructions([createProgramIx, deployIx], tx),
        tx => signAndSendTransaction(client, tx),
    );
...
export async function createBuffer(
    ...
    if (dataLenth >= REALLOC_LIMIT) {
        ...
        while (offset < dataLenth) {
            ...
            instructions.push(getExtendInstruction({ account: buffer, authority, length }));
            offset += length;
        }
    }
    await pipe(
...
export async function createMetadata(
    client: Client,
    input: PartialExcept<InitializeInput, 'metadata' | 'authority' | 'seed' | 'program'> & {
        payer?: TransactionSigner;
    },
): Promise<void> {
    const { payer = input.authority, ...initializeInput } = input;
    const data = (isOption(input.data) ? unwrapOption(input.data) : input.data) ?? new Uint8Array([]);
