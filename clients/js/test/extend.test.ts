...
import { ACCOUNT_HEADER_LENGTH, getExtendInstruction, getSetAuthorityInstruction } from '../src';
import {
...
test('the explicit authority of a canonical metadata account can extend it', async t => {
    ...
    await pipe(
        ...
        tx => appendTransactionMessageInstructions([setAuthorityIx, transferIx, extendIx], tx),
        tx => signAndSendTransaction(client, tx),
    );
