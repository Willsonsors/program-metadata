...
export function packDirectData(input: {
    ...
    const { content, compression = Compression.Zlib, encoding = Encoding.Utf8 } = input;
    const data = pipe(
        content,
...
export function packUrlData(input: {
    ...
    const { url, compression = Compression.Zlib, encoding = Encoding.Utf8 } = input;
    const data = pipe(
        url,
        c => encodeData(c, encoding),
        c => compressData(c, compression),
...
export function unpackDirectData(input: Omit<PackedData, 'dataSource'>): string {
    return pipe(
        input.data,
...
export async function unpackAndFetchUrlData(input: Omit<PackedData, 'dataSource'>): Promise<string> {
    const url = unpackUrlData(input);
    const response = await fetch(url);
    return await response.text();
}

export function unpackExternalData(data: ReadonlyUint8Array): UnpackedExternalData {
...
export async function unpackAndFetchExternalData(
    ...
    const account = await fetchEncodedAccount(input.rpc, unpackedExternalData.address);
...
export async function unpackAndFetchData(input: PackedData & { rpc: Rpc<GetAccountInfoApi> }): Promise<string> {
    ...
    return await Promise.all(
        accounts.map(async account => {
            switch (account.data.dataSource) {
                ...
                case DataSource.External: {
                    const accountIndex = unpackedExternalAccounts.findIndex(acc => acc.address === account.address);
                    return unpackFetchedExternalData({
                        compression: account.data.compression,
                        encoding: account.data.encoding,
                        account: fetchedExternalAccounts[accountIndex],
                        unpackedExternalData: unpackedExternalAccounts[accountIndex].unpacked,
                    });
                }
            }
        }),
...
export function compressData(data: ReadonlyUint8Array, compression: Compression): ReadonlyUint8Array {
    switch (compression) {
...
export function uncompressData(data: ReadonlyUint8Array, compression: Compression): ReadonlyUint8Array {
    switch (compression) {
        case Compression.None:
            return data;
        case Compression.Gzip:
...
export function encodeData(data: string, encoding: Encoding): ReadonlyUint8Array {
    switch (encoding) {
        case Encoding.None:
...
export function decodeData(data: ReadonlyUint8Array, encoding: Encoding): string {
    switch (encoding) {
        case Encoding.None:
