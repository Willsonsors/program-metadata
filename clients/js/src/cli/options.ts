...
export const keypairOption = new Option('-k, --keypair <path>', 'Path to keypair file.').default(
    undefined,
    'solana config',
);
...
export const priorityFeesOption = new Option(
    ...
    .default('100000')
    .argParser((value: string | undefined) => (value !== undefined ? (BigInt(value) as MicroLamports) : undefined));

export type RpcOption = { rpc?: string };
export const rpcOption = new Option('--rpc <string>', 'RPC URL.').default(undefined, 'solana config or localhost');
...
export const textOption = new Option('--text <content>', 'Direct content to upload (creates a "direct" data source).');

export type UrlOption = { url?: string };
export const urlOption = new Option('--url <url>', 'The url to upload (creates a "url" data source).');
...
export const compressionOption = new Option('--compression <compression>', 'Describes how to compress the data.')
...
export const encodingOption = new Option('--encoding <encoding>', 'Describes how to encode the data.')
    .choices(['none', 'utf8', 'base58', 'base64'])
    .default(Encoding.Utf8, 'utf8')
    .argParser(encodingParser);

export type FormatOption = { format?: Format };
export const formatOption = new Option('--format <format>', 'The format of the provided data.')
...
export const outputOption = new Option('-o, --output <path>', 'Path to save the retrieved data.');

export type NewAuthorityOption = { newAuthority: Address };
export const newAuthorityOption = new Option('--new-authority <new-authority>', 'The new authority to set')
    .makeOptionMandatory()
