...
import { GlobalOptions, nonCanonicalReadOption, NonCanonicalReadOption, outputOption, OutputOption } from '../options';
import { CustomCommand, getKeyPairSigners, getReadonlyClient, writeFile } from '../utils';
...
export function setFetchCommand(program: CustomCommand): void {
    program
        ...
        .addOption(new Option('--raw', 'Output raw data in hexadecimal format.').default(false))
        .action(doFetch);
}

type Options = NonCanonicalReadOption & OutputOption & { raw: boolean };
async function doFetch(seed: Seed, program: Address, _: Options, cmd: CustomCommand) {
    ...
    try {
        ...
        if (options.output) {
            ...
            logSuccess(`Metadata content saved to ${picocolors.bold(options.output)}`);
        } else {
            console.log(content);
        }
    } catch (error) {
