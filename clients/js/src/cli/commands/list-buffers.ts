...
type Options = {};
export async function doListBuffers(authorityArgument: Address | undefined, _: Options, cmd: CustomCommand) {
...
function getAuthorityFilter(authority: Address): GetProgramAccountsMemcmpFilter {
    return {
        memcmp: {
            bytes: pipe(authority, getAddressEncoder().encode, getBase64Decoder().decode) as Base64EncodedBytes,
            encoding: 'base64',
